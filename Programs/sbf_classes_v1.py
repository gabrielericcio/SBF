...
def run_part1(rms=0, remove_bkg=False, star_image=False) :
    ...
    #Open image for star selection
    
    if star_image==True:
        starspath=f"../INPUT/IMAGES/{gxyid}/cutout_stars.fits"
        starspath_weight=f"../INPUT/IMAGES/{gxyid}/cutout_stars_sig.fits"
        
        hdus = fits.open(starspath, do_not_scale_image_data=True)
        hdus.info()
        stars_data = hdus[img_ext].data
        
        hdusw = fits.open(starspath_weight, do_not_scale_image_data=True)
        hdusw.info()
        stars_wht_data = hdusw[img_ext].data
    ...
    # fits.writeto(galimg, image_data, overwrite=True)
    ...
    if(rms):
        ...
        if star_image==True:
            stars_wht_data=sex_mask/np.square(stars_wht_data)
            fits.writeto(f"../OUTPUT/{gxyid}/stars_weights.fits", stars_wht_data, overwrite=True)
            hdusw.close()
    ...
    #FOR HSC DATA
        ...
        if star_image==True:
            stars_wht_data=sex_mask/stars_wht_data
            fits.writeto(f"../OUTPUT/{gxyid}/stars_weights.fits", stars_wht_data, overwrite=True)
            hdusw.close()
    ...
    out=sew(galimg) #The run is done on image_data, that is the image
    ...
    copyfile(out_compact["catfilepath"],"../OUTPUT/"+'test_stars111.cat')
    ...
    fwhm_list=tsex_cpt[(class_star>0.9) & (mag<=23) & (flags==0)]['FWHM_IMAGE']
    print("Number of objects chosen to calculate FWHM: ",len(fwhm_list), '\n\n')
    #in case you wanna use another image to estimate the fwhm
    ...
    if star_image==True:
        ...
        sewpyconf={"PARAMETERS_NAME":"../INPUT/UTILS/default.param",
                              ...
                              "MAG_ZEROPOINT": magzp,
                              "PIXEL_SCALE" : gal.plate_scale,
                              "WEIGHT_IMAGE":f"../OUTPUT/{gxyid}/stars_weights.fits"}
        ...
        out_compact=sew_compact(mback_image)
        copyfile(out_compact["catfilepath"],"../OUTPUT/" +'test_stars.cat')
    
        #Load the Sextractor file generated by sewpy
        
        sex_cpt = Table.read(out_compact["catfilepath"], format='ascii.sextractor')
        print('sex_cpt',len(sex_cpt))
        print('sex_cpt', len(sex_cpt))
        sex_cpt=sex_cpt[sex_cpt['MAG_AUTO']<35.0]
        mag_s=sex_cpt['MAG_AUTO']
        class_star_s=sex_cpt['CLASS_STAR']
        flags_s=sex_cpt['FLAGS']
        
        # Selection of FWHM objects (stars)
        
        fwhm_list=sex_cpt[(class_star_s>0.5) & (mag_s<=23) & (flags_s==0)]['FWHM_IMAGE']
        print("Number of objects chosen to calculate FWHM from other image: ",len(fwhm_list))
    ...
    #Isophote generation
    tini_i=time.time()
    isolist = ellipse.fit_image(minsma=minsma0, maxsma=maxsma0, maxit=maxit, maxgerr=0.5, fflag=0.9,
...
def run_part2(alpha) :
    ...
    starimg="../INPUT/"+'/IMAGES/'+gxyid+'/'+'cutout_stars.fits'
