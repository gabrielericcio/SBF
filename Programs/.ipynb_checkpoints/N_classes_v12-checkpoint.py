...
def run_part1(rms=0, remove_bkg=False) :
    ...
    #GALAXY FILE PATH
    #gxy='/Users/mik/Desktop/Astrowork/LSST/Nandini/I0745/HSC-I-9590-pdr2_wide.fits'
    #gxy='/Users/mik/Desktop/Astrowork/LSST/Nandini/NGC5831/ngc5831.fits'
    gxy=gal.gxyfilepath
    print(gxy)
    ...
    ########################################################
    #Read instrument mask if so instructed in init file
    flag_instr_mask=1
    ...
    #Get the four corners backopgrund
    ...
    mag=tsex['MAG_AUTO']
    tmask=tsex[(rgc>=maskin) & (rgc <=maskout) & (mag<magcut)] #Choose objects within a certain radius of the galaxy centre
                                                               #which are brighter than a cutoff magnitude set by the user###Plot class star vs mag auto
    '''
    ax1 = plt.subplot(211)
    ax1.plot(tsex['MAG_AUTO'],tsex['CLASS_STAR'],'oy',label='Ext_det')
    
    ax1.plot(tmask['MAG_AUTO'],tmask['CLASS_STAR'],'og',label='Ext_masked')
    ax1.set_ylabel('Class star')
    ax1.set_xlabel('Mag')
    ax1.axvline(x = magcut)#, color = 'r', linestyle = 'dashed')
    plt.legend()
    '''
    ...
    # print(center)
    width=mask_scale_ext*tmask['A_WORLD']#*3600/plate_scale
    height=mask_scale_ext*tmask['B_WORLD']#*3600/plate_scale
    for j in range(len(tmask)):
        ...
        #print(((region.to_pixel(wgxy)).to_mask(mode='center')))
    ...
    #tsex_cpt=tsex_cpt[mag<35]
    tmask_cpt=tsex_cpt[(rgc>=maskin) & (rgc <=maskout) & (mag<magcut_cpt) & (class_star>0.8)]  
    
    '''
    ###Plot class star vs mag auto
    ax2 = plt.subplot(212, sharex=ax1)
    ax2.plot(tsex_cpt['MAG_AUTO'],tsex_cpt['CLASS_STAR'],'oy',label='Cpt_det')
    ax2.plot(tmask_cpt['MAG_AUTO'],tmask_cpt['CLASS_STAR'],'og',label='Cpt_masked')
    ax2.set_ylabel('Class star')
    ax2.set_xlabel('Mag')
    ax2.axvline(x = magcut_cpt)#, color = 'r', linestyle = 'dashed')
    
    plt.legend()
    #plt.show(block=False)
    ##plt.clf() #Clear plot for further use
    '''
    print('Generating the mask of all compact objects within ',maskin,' <=Rgc(arcsec)<=',maskout,' and brighter than ',magcut_cpt)
    
    #Choose only compact objects within a certain radius of the galaxy centre
    #which are brighter than a cutoff magnitude set by the user
    ...
    #     region_file="../OUTPUT/"+gxy_name+".reg"
    #     r= regions.open(region_file).as_imagecoord(hdu[img_ext].header)
    #     mask=r.get_mask(hdu=hdu[img_ext])
    #     mask_data=np.multiply(mask,1)
    #     #Masked image data
    ...
    #sampell = EllipseSample(masked_data, sma=smagxy, geometry=geogxy, sclip=3, nclip=5,
    #                        integrmode='bilinear')
    ...
    if (len(t)>0) :
        ...
        # fits.writeto("../OUTPUT/"+gxyid+'/'+gxy_name+'.fits', image_data,  header=hdu[img_ext].header, overwrite=True)
        # print("***********************&&&&&&&&&&*****************")
